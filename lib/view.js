...
exports.viewAllRoles = () => {
    connection.query('SELECT a.acting_role_id, a.title, a.salary, d.dept_name AS dept_name FROM acting_role AS a INNER JOIN department AS d ON a.dept_id = d.dept_id', function (err, res) {
        if (err) throw err;
        console.table(res);
        app.start();
    });
